...
export const useStore = create<AppState>((set) => ({
  ...
  addEntity: (entity) =>
    set((state) => ({
      entities: [
        ...state.entities,
        { ...entity, id: `e${Date.now()}`, createdAt: Date.now() },
      ],
    })),
  addEvidence: (evidence) =>
    set((state) => ({
      evidence: [
        ...state.evidence,
        { ...evidence, id: `ev${Date.now()}`, createdAt: Date.now() },
      ],
    })),
  addEvent: (event) =>
    set((state) => ({
      events: [
        ...state.events,
        { ...event, id: `evt${Date.now()}`, createdAt: Date.now() },
      ],
    })),
  addClaim: (claim) =>
    set((state) => ({
      claims: [
        ...state.claims,
        { ...claim, id: `c${Date.now()}`, createdAt: Date.now() },
      ],
    })),
